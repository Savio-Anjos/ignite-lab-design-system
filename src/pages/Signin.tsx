...
import { MeuCheckbox } from "../components/MeuCheckbox";
   ...
   const [isUserSignedIn, setIsUserSignedIn] = useState(false)
    ...
    async function handleSignIn(event: FormEvent) {
      ...
      await axios.post('/sessions', {
        email: 'savioanjos2020@gmail.com',
        password: '12345678',
      })
      ...
      <Text size="lg" className='text-gray-400 mt-1'>Faça login e comece a usar!</Text>
        ...
        <form onSubmit={handleSignIn} className='flex flex-col gap-4 items-stretch w-full max-w-sm mt-10'>
          { isUserSignedIn && <Text>Login realizado!</Text>}
           ...
           <label htmlFor='password' className='flex flex-col gap-3'>
             <Text  className='font-semibold'>Endereço sua senha</Text>
            <TextInput.Root>
                <TextInput.Icon>
                  <Lock/>
                </TextInput.Icon>
        ...
        <footer className='flex flex-col items-center gap-4 mt-8'>
           ...
           <Text asChild size='sm'>
            ...
            <Text asChild size='sm'>
            <a href='' className="text-gray-400 underline hover:text-gray-200">Não possui conta? Crie uma?</a>
            </Text>
        </footer>
